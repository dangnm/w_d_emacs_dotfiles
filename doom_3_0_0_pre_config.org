...
- Or run ~/.config/emacs/bin/doom sync
* Helpers
** Basic helpers
...
#+BEGIN_SRC emacs-lisp
;; Add asdf shims to exec-path so Emacs can find tsserver
(add-to-list 'exec-path (expand-file-name "~/.asdf/shims"))
(setenv "PATH" (concat (expand-file-name "~/.asdf/shims") ":" (getenv "PATH")))
;; .tsx files: rjsx-mode (installed via Doom javascript module)
(add-to-list 'auto-mode-alist '("\\.tsx\\'" . rjsx-mode))
;; .ts files: typescript-mode
(add-to-list 'auto-mode-alist '("\\.ts\\'" . typescript-mode))
#+END_SRC
** Agent shell config
Packages must be declared in ~~/.doom.d/packages.el~. Add these 3 lines then run ~doom sync~ from terminal:
#+BEGIN_SRC emacs-lisp :tangle no
(package! shell-maker :recipe (:host github :repo "xenodium/shell-maker"))
(package! acp         :recipe (:host github :repo "xenodium/acp.el"))
(package! agent-shell :recipe (:host github :repo "xenodium/agent-shell"))
#+END_SRC
Make sure ~ANTHROPIC_API_KEY~ is exported in your shell and run ~doom env~.
#+BEGIN_SRC emacs-lisp
;; Add asdf nodejs bin dynamically so Emacs can find claude-agent-acp
(when (executable-find "asdf")
  (let ((nodejs-bin (string-trim (shell-command-to-string "asdf where nodejs 2>/dev/null"))))
    (unless (string-empty-p nodejs-bin)
      (let ((bin-path (concat nodejs-bin "/bin")))
        (add-to-list 'exec-path bin-path)
        (setenv "PATH" (concat bin-path ":" (getenv "PATH")))))))

;; agent-shell requires Emacs 29+ (uses defvar-keymap)
(when (and (>= emacs-major-version 29)
           (locate-library "agent-shell"))
  (autoload 'agent-shell "agent-shell" "Launch an LLM agent shell." t)
  ...
  (with-eval-after-load 'agent-shell
    ...
    (setq agent-shell-inherit-environment t))

  ;; SPC t a → agent-shell  |  C-u M-x agent-shell → new session
  (map! :leader :desc "Agent shell" "t a" #'agent-shell))
#+END_SRC
* Tips
** Emacs
